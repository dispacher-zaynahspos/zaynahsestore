      ...
      >
        ...
        <div className={`relative ${aspectClass} w-full overflow-hidden bg-gray-50`}>
          ...
          {(selectedSwatchImage || hoveredImage) ? (
            <Image
              src={imageErrors[selectedSwatchImage || hoveredImage!] ? fallbackPlaceholder : (selectedSwatchImage || hoveredImage!)}
              alt={product.name}
              fill
              ...
              unoptimized={true}
              onError={() => {
                const target = selectedSwatchImage || hoveredImage!;
                setImageErrors(prev => ({ ...prev, [target]: true }));
              }}
            />
          ) : (
            <>
              <Image
                src={imageErrors[initialImage] ? fallbackPlaceholder : initialImage}
                alt={product.name}
                fill
                sizes="(max-width: 768px) 50vw, 25vw"
                ...
                unoptimized={true}
                onError={() => {
                  setImageErrors(prev => ({ ...prev, [initialImage]: true }));
                }}
              />
              {secondImage && (
                <Image
                  src={imageErrors[secondImage] ? fallbackPlaceholder : secondImage}
                  ...
                  unoptimized={true}
                  onError={() => {
                    setImageErrors(prev => ({ ...prev, [secondImage]: true }));
                  }}
                />
              )}
            </>
          ...
          <div className="absolute top-2 left-2 flex flex-col gap-1 z-10 items-start pointer-events-none">
            ...
            {/* Show low stock limited badge on cards */}
          ...
          {/* Style 1 / Style 5: Premium Elessi Floating Circular Buttons */}
          {(cardStyle === 'style1' || cardStyle === 'style5') && (
            <div className="absolute right-2 top-2.5 flex flex-col gap-2 z-20 transition-all duration-300 opacity-100 md:opacity-0 md:group-hover:opacity-100">
              {showWishlist && (
                <button
                  type="button"
                  onClick={handleToggleWishlist}
                  className="flex h-9 w-9 items-center justify-center rounded-full bg-white dark:bg-[#16162a] shadow-md border border-gray-150 dark:border-gray-800 text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-white/10 hover:text-[#e94560] dark:hover:text-[#e94560] transition-all cursor-pointer active:scale-90"
              ...
              {cardStyle === 'style1' && showQuickcart && (
                <button
                  type="button"
                  onClick={handleAddToCart}
                  className="flex h-9 w-9 items-center justify-center rounded-full bg-white dark:bg-[#16162a] shadow-md border border-gray-155 dark:border-gray-800 text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-white/10 hover:text-[#e94560] dark:hover:text-[#e94560] transition-all cursor-pointer active:scale-90"
          ...
          {cardStyle === 'style5' && (
            <div className="absolute bottom-0 left-0 right-0 bg-[#e94560]/95 backdrop-blur-xs text-white text-[10px] py-2 px-3 flex items-center justify-between font-bold z-10">
              <span className="uppercase tracking-wider text-[8px] opacity-90">Sale Ends In:</span>
          ...
          {/* Floating cart button for Style 5 overlapping countdown bar */}
          {cardStyle === 'style5' && showQuickcart && (
            <button
              type="button"
              onClick={handleAddToCart}
              className="absolute bottom-2.5 right-2.5 z-20 flex h-11 w-11 items-center justify-center rounded-full bg-white dark:bg-[#16162a] shadow-lg border border-gray-150 dark:border-gray-800 text-[#10b981] hover:bg-gray-50 dark:hover:bg-white/10 hover:scale-105 active:scale-95 transition-all cursor-pointer"
              title={product.hasVariants ? "Choose Options" : "Add to Cart"}
            >
          ...
          <div className="w-full opacity-100 max-h-[200px] md:max-h-0 md:opacity-0 group-hover:md:max-h-[200px] group-hover:md:opacity-100 transition-all duration-300 ease-in-out overflow-hidden mt-0 group-hover:mt-2">
            ...
            {cardStyle === 'style5' && (
              <div className="mt-3 space-y-1.5 w-full">
                <div className="flex items-center justify-between text-[9px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-wide">
                  ...
                  <span className="text-[#10b981]">Only {Math.max(2, 20 - Math.round((Math.max(30, (product.id.charCodeAt(0) % 40) + 45)) / 5))} Left!</span>
                </div>
            ...
            {cardStyle === 'style3' && showQuickcart && (
              <button
                type="button"
                onClick={handleAddToCart}
                className="mt-3 w-full py-2 bg-[#1a1a2e] dark:bg-white/5 hover:bg-[#e94560] dark:hover:bg-[#e94560] text-white text-[10px] font-black uppercase tracking-wider rounded-xl transition-all shadow-sm flex items-center justify-center gap-1.5 active:scale-[0.98] cursor-pointer border-none"
              >
                <ShoppingCart className="h-3.5 w-3.5" />
                {product.hasVariants ? "Choose Options" : "Add to Cart"}
              </button>
            ...
            {cardStyle === 'style4' && showQuickcart && (
              <button
                type="button"
                onClick={handleAddToCart}
                className="mt-2 text-[10px] font-black uppercase tracking-widest text-gray-505 dark:text-gray-450 hover:text-black dark:hover:text-white underline transition-colors cursor-pointer border-none bg-transparent py-1 w-full text-center"
              >
